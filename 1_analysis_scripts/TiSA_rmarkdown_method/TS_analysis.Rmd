---
title: "TimeSeries Analysis – SARS-CoV2 Mock vs Infected"
author: "Yohan Lefol"
...
```{r knitr_ggplot, include=FALSE}
#Added from Original to change ggplot theme
...
#This writes example data to directory
#Use 'PBMC', 'murine', or 'celegans' to load the different experimental data
#Note that celegans is a heavy dataset and will take time to write to directory.
#write_example_data_to_dir('COVID')

#Give names to saved object and name of results repository
name_result_folder<-'TS_results_COVID/'
obj_name<-'timeSeries_obj_COVID.Rdata'

#Path to count data and sample data respectively
my_path_data<-'data/COVID/raw_counts_TS'
my_path_sample_dta<-'data/COVID/sample_file.csv'
...
name_control<-'mock' #Name of control as seen in the sample file
name_experiment<-'infected' #Name of experiment as seen in the sample file
graphic_vector<-c("#e31a1c","cornflowerblue") #Pre-set colors for the groups
...
genes_of_interest <- c("LOC112268133", "CLIC4", "NDNF", "MGLL", "CTH", "ACE2", "ACE", "TMPRSS2")
...
if(nrow(gene_matrix_interest)>0){
  ...
  target_file<-paste0(name_result_folder,'genes_of_interest/',selected_gene,'.png')
...
wrapper_cluster_trajectory(TS_object,ts_data,mean_ts_data,yaxis_name='scaled mean counts',log_TP=log_tp_traj,plot_name=paste0(name_result_folder,'/PART_results/Ctraj'))
...
if(length(unique(ts_data$timepoint))==1){
  plt_clust<-plot_cluster_traj(object = TS_object,ts_data = ts_data,ts_mean_data = mean_ts_data,yaxis_name='scaled mean counts', num_col=1,rem_legend_axis=TRUE,log_TP=log_tp_traj)
}else{
  sub_ts<-ts_data[ts_data$cluster==target_clust,]
  sub_mean<-mean_ts_data[mean_ts_data$cluster==target_clust,]
  plt_clust<-plot_cluster_traj(object = TS_object,ts_data = sub_ts,ts_mean_data = sub_mean,yaxis_name='scaled mean counts', num_col=1,rem_legend_axis=TRUE,log_TP=log_tp_traj)
}
